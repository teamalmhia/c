  ...
  $('.public_image').change(function() {
        ...
        $.ajax({
          ...
          success: function(data) {
            if (data.status == 0) {
              ...
              socket.emit('publicMsg', data, function(response) {
    ...
    socket.on('appendWall', function(data) {
      ...
      if (data.status == 0) {
        ...
        var final = '<li class="single-chat-line text-left post-' + data.post.id + '" style="border-bottom: 2px; margin-bottom:0px;padding:0;"><div style="border: 1px solid #cacaca; width: 36px; height: 38px; margin-left: 1px; margin-top: 1px; " class="user-avatar pull-left"><a class="single-user-panel" visitor_id="' + data.post.user_id + '"><img src="' + image + '" alt="' + username + '" class="userIMG"></a></div>';
        var display = (user.type == "admin" || user.group.delete_wall == 1) ? 'display:block' : 'display:none';
